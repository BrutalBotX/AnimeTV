...
//    webviewInitConfig(webView2);
    ...
    AppGraph.INSTANCE.init();
    PipeBridgeInit.init(activity);
  ...
  public void initVideoView(){
    ...
    videoDataSourceFactory= (s, transferListener) -> {
      ...
      try{
        ...
        else if (Conf.SOURCE_DOMAIN==9 && videoReferer != null){
...
//    }
    ...
    @JavascriptInterface
    public void extGetView(int extId, String slug, int cbId){
      AsyncTask.execute(() -> {
        try {
          int anilistId = Integer.parseInt(slug.replaceAll("[^0-9]", ""));
          String result = AnililiBridge.getView(anilistId);
          final String js = "__EXT_DISPATCH(" + cbId + "," + JSONObject.quote(result) + ");";
    ...
    @JavascriptInterface
    public void extLoadVideo(int extId, String slug, int epNum, String subType, int cbId){
      AsyncTask.execute(() -> {
        try {
          int anilistId = Integer.parseInt(slug.replaceAll("[^0-9]", ""));
          String result = AnililiBridge.getVideoSources(anilistId, epNum, subType);
          final String js = "__EXT_DISPATCH(" + cbId + "," + JSONObject.quote(result) + ");";
          activity.runOnUiThread(() -> webView.evaluateJavascript(js, null));
        } catch (Exception e) {
    ...
    @JavascriptInterface
    public void extSearch(int extId, String query, int page, int cbId){
      AsyncTask.execute(() -> {
        try {
          String result = query.isEmpty() ?
              AnililiBridge.getPopular(page) :
              AnililiBridge.search(query, page);
          final String js = "__EXT_DISPATCH(" + cbId + "," + JSONObject.quote(result) + ");";
          activity.runOnUiThread(() -> webView.evaluateJavascript(js, null));
        } catch (Exception e) {
          Log.e(_TAG, "extSearch error: " + e.getMessage());
          final String js = "__EXT_DISPATCH(" + cbId + ",\"[]\");";
          activity.runOnUiThread(() -> webView.evaluateJavascript(js, null));
        }
      });
